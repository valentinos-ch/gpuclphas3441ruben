...
and national security. Through the NPT, the IAEA has set up safeguards dictating what
...
treaties: the capabilities of the equipment is an example
procedure. The treaties specify what information cannot
be shared with the inspectors; they do not however specify methods the 
inspectors can use to verify the dismantlent of the weapon. The 
information barrier is most likely
implemented as a computer program placed between the statistical
...
these methods and determine if it is possible to confidently verify
...
State (NWS) with a smaller stockpile could attain. This
...
methods be used in conjunction with the
conventional approaches. These proposed methods include the use of
pit-stuffing, military GPS, anti-evidence seals and the ``town crier'' method.
...
For any fissile weapon, radiation is emitted. The detection of any
such radiation provides information about the material without needing
to directly interact with the inspected material. This is known as
passive detection. \citep{hippel1990} When such a detection method is
used, the detection has to compete with natural background
radiation. Such a competition means that the detection process has to
wrestle with statistical complications. In the fortunate cases whereby
the radiation emitted is much greater than the natural background
radiation, no complication is needed to get the information.

Statistical analysis, on the other hand, provides an important tool in
the case that the radiation signal is comparable, or indeed smaller,
than the background noise. The procedure rests upon the random walk --
by collecting the signals over a period of time, it is possible to
detect the presence of a signal over the random noise.

The above discussion describes the problem of detection and deals with
the issue using purely natural constraints. Practically, the
detectability of nuclear warheads is reliant upon four factors. These
factors are the warhead's design, the technique used for detection,
the sensitivity of the detectors and any material between the warhead
and radiation. \citep{hippel1990}

The advantages of passive detectors are: they require less electrical
...
radiation from reaching the detector. \citep{nuclearDetArch2009}

If the type of detection mechanism is restricted to the conventional
choices of gamma rays and neutrons, then further criteria that affect
the detection process can be affixed. For example, it is possible to
measure the wavelength of gamma rays to high accuracy, while it is not
currently possible to do the same with neutrons. The sensitivity of
gamma rays means that measuring the radiation spectrum from the
nuclear bomb may reveal NPT-sensitive information. Neutrons, on the
other hand, tend to indicate only presence or not. 

The features of materials which are relevant to neutron and gamma ray
detection include:
\begin{enumerate}
...
\section{Conclusion}
This commission recommends establishing a department within the
...
facilities would have neutral international teams of inspectors observing the construction
...
and countries would pay a reduced expenditure for nuclear weapon storage
...
dismantlement facility inside the host nation as this ensures that building
specifications are being met. This minimises the
opportunity for the host nation to to build 
...
Before dismantlement, the parameters for the quantities of weapons grade Plutonium
...
physics package. Safety procedures would need to be devised for the
...
the risk of an insider threat. As mentioned before, inspectors could
...
able to work with a minimum number of restrictions in this area, but they
...
authenticating any equipment supplied by the host. The newly established
IAEA department could design a universally used tag and seal system 
enabling the inspector team to ensure confidence levels during transportation
to the dismantlement facilities. It is highly recommended
...
neutrons and gamma rays from the warhead, in combination with muon
tomography. Strategies for monitoring
